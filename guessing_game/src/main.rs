...
fn guess_game(random_num: &u8, mut counter: u8, trial: &u8) {
    ...
    let converted_value = guess.trim().parse::<u8>();
    ...
    if counter == *trial {
        println!("!!!!!!!!!!!Game over!!!!!!!!!!!!!!!");
        println!("the secret number was {random_num}")
    } else {
        counter = counter + 1;
        match random_num.cmp(&converted_value.unwrap()) {
            Ordering::Greater => {
                println!("too small, try again \n\n\n");
                println!("You have left {} chance", trial - counter);
